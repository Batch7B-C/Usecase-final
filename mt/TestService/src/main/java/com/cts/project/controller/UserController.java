...
public List<User> getAllUsers(){
	...
	@GetMapping("/findByUsernameAndPassword/{username}/{password}")
	 ...
	 public ResponseEntity<User> findByUsernameAndPassword(@RequestBody User user,@PathVariable("username") String username,@PathVariable("password") String password)

	 {
		...
		header.setBasicAuth(configuration.getAuthUsername(), configuration.getAuthPassword());
		HttpEntity<String> requestEntity = new HttpEntity<String>(header);
		ResponseEntity<User> list = restTemplate.exchange("http://localhost:9099/UserPortal/findByUsernameAndPassword/{username}/{password}",
              HttpMethod.POST, requestEntity, new ParameterizedTypeReference<User>() { });
	//	return list;
